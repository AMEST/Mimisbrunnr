...
public class HandlePageTemplateErrorsAttributeTests
{
    private static async Task<ExceptionContext> InvokeFilterAsync(Exception exception)
    {
        ...
        await new HandlePageTemplateErrorsAttribute().OnExceptionAsync(context);
        return context;
    }

    [Fact]
    public async Task Should_Return404_WhenPageTemplateNotFound()
    {
        var context = await InvokeFilterAsync(new PageTemplateNotFoundException());
