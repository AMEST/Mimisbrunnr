...
internal static class ConfluenceContentPreprocessing
{
    ...
    private static readonly Regex FindAttachmentMacroRegEx = new Regex("<ri:attachment\\sri:filename=\"([^\"]*)\"\\s\\/>", RegexOptions.Compiled);
    private static readonly Regex FindKDrawIOMacroRegEx = new Regex("<ac:structured-macro\\sac:name=\"kontur-draw-io\"[^>]*>.*(?=<\\/ac:structured-macro>)<\\/ac:structured-macro>", RegexOptions.Compiled);
    private static readonly Regex FindDrawIOMacroRegEx = new Regex("<ac:structured-macro\\sac:name=\"drawio\"[^>]*>[\\s\\n]*(?:<ac:parameter[^>]*>[^>]*>)*<\\/ac:structured-macro>", RegexOptions.Compiled);
    private static readonly Regex FindDrawIOMacroSubRegEx = new Regex("ac:name=\"diagramName\"\\>([^<]*)<\\/ac:parameter>", RegexOptions.Compiled);
